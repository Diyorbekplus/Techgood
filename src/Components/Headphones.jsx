...
const Headphones = () => {
    const {headphonesCategory, headphonesList,ToggleLikes, addToCompare,addToCart,showCategoriesSection, hideCategories, inputValue } = useGlobalContext();
    const keys = ['brand', "title"]
    const search = (info) => {
        return info.filter((item) =>
        keys.some(key => item[key].toUpperCase().includes(inputValue.toUpperCase())))
     }
    const [data, setData] = useState(search(headphonesList));
    useEffect(() => {
        setData(search(headphonesList))
    }, [inputValue])
    const filterProducts = (cartItem) => {
        hideCategories()
        const result = headphonesList.filter((currData) => {
