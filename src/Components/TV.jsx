...
const TV = () => {
    const {tvCategory, tvList, ToggleLikes, addToCompare,addToCart,showCategoriesSection, hideCategories, inputValue  } = useGlobalContext();
    const keys = ['brand', "title"]
    const search = (info) => {
        return info.filter((item) =>
        keys.some(key => item[key].toUpperCase().includes(inputValue.toUpperCase())))
     }
    const [data, setData] = useState(search(tvList));
    useEffect(() => {
        setData(search(tvList))
    }, [inputValue])
    
    const filterProducts = (cartItem) => {
        hideCategories()
        const result = tvList.filter((currData) => {
