...
const Laptops = () => {
    const {laptopsCategory, laptopsList, ToggleLikes, addToCompare,addToCart,showCategoriesSection, hideCategories,inputValue } = useGlobalContext();
    const keys = ['brand', "title"]
    const search = (info) => {
        return info.filter((item) =>
        keys.some(key => item[key].toUpperCase().includes(inputValue.toUpperCase())))
     }
    const [data, setData] = useState(search(laptopsList));
    useEffect(() => {
        setData(search(laptopsList))
    }, [inputValue])
    
    const filterProducts = (cartItem) => {
        hideCategories()
        const result = laptopsList.filter((currData) => {
